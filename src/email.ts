...
import { getClassifiedSummaries } from "./summarizing.js";
import { clearPageContent, clearScrapedArticles } from "./utils/cleanup.js";
import { EMAIL_CONFIG } from "./utils/constants.js";
import { logger } from "./utils/logging.js";
import { stripCodeMarkers } from "./utils/parsing.js";
import {
  ArticleSource,
  ProcessedArticles,
  RankedArticle,
...
export const sendDailyNewsReport = async (): Promise<void> => {
  return EmailService.getInstance().sendDailyNewsReport();
};

export const sendDailyJamstockexReport = async (): Promise<void> => {
  return EmailService.getInstance().sendDailyJamstockexReport();
};
...
export const sendEmail = async (
  ...
  return EmailService.getInstance().sendEmail(html, subject, maxRetries);
};
...
export const generateDailyNewsHtml = (
  ...
  return EmailService.getInstance().generateDailyNewsHtml(sections, priority);
};

export const generateDailyJamstockexHtml = (
  sections: ArticleSource[]
): string => {
  return EmailService.getInstance().generateDailyJamstockexHtml(sections);
};

export const generateDailyStockSummaryHtml = (
  sections: StockData[]
): string => {
  return EmailService.getInstance().generateDailyStockSummaryHtml(sections);
};
